...
void moveLeft(int spd){
  if (spd == 0) {
    analogWrite(LEFT_MOTOR_REV_PIN, 0);
    analogWrite(LEFT_MOTOR_FWD_PIN, 0);  
  } else if (spd > 0) {
    analogWrite(LEFT_MOTOR_REV_PIN, 0);
    analogWrite(LEFT_MOTOR_FWD_PIN, spd);
  } else {
    analogWrite(LEFT_MOTOR_REV_PIN, spd*(-1));
    analogWrite(LEFT_MOTOR_FWD_PIN, 0);
  }
}
...
void moveRight(int spd){
  if (spd == 0){
    analogWrite(RIGHT_MOTOR_REV_PIN, 0);
    analogWrite(RIGHT_MOTOR_FWD_PIN, 0);
  } else if (spd > 0) {
    analogWrite(RIGHT_MOTOR_REV_PIN, 0);
    analogWrite(RIGHT_MOTOR_FWD_PIN, spd);
  } else {
    analogWrite(RIGHT_MOTOR_REV_PIN, spd*(-1));
    analogWrite(RIGHT_MOTOR_FWD_PIN, 0);
  }
}
